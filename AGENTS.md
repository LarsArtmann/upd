...
- **File-naming trap**: the registry client is `npm.go` (+ `npm_test.go`). Commit `7402f06` renamed *documentation references* to `pnpm.go` but never renamed the actual file — older status reports may say `pnpm.go`; that file does not exist. Don't "fix" code to match those references.
...
- **gobwas/glob v1.0.0 API rename**: upstream changed `glob.Glob` (interface, v0.2.x) to `*glob.Pattern` when the v1.0.0 tag was refreshed; the `ebd9b1b` dependency bump shipped without updating `manifest.go`, breaking the build (fixed 2026-09-25). If other deps get major-bumped by automation, expect API renames to need code changes — don't trust dep-only commits to be compile-clean.
