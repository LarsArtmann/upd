...
7. **Fetch** packuments concurrently (semaphore bounded by `-c`, default 8) from registry (`engine.go`, `pnpm.go`). Names are lowercased before fetch. `RegistryClient` takes `*Config` (supports custom registry URL, timeout, retries). Retry logic: 429/5xx retried with exponential backoff + `Retry-After` header support. HTTP transport tuned (MaxIdleConns, IdleConnTimeout). Context is signal-aware (SIGINT/SIGTERM cancellation).
...
- **Packument**: NPM registry JSON for one package (`dist-tags`, `versions`, ...). Held as raw bytes in `pnpm.go`.
...
- **Retry logic**: `pnpm.go` retries 429/5xx responses with exponential backoff (1s base, 30s cap). `Retry-After` header honored if present. Non-retryable errors (404, network errors) fail immediately. The `RegistryClient.sleep` field (type `sleeper`) abstracts delays so tests run without real sleeps — `newTestEngine` sets it to a no-op; npm_test.go tests can capture delays to assert backoff timing.
...
- **`go-error-family` adoption**: All 13 domain sentinels (`errors.go`) use `errorfamily.NewRejection/NewCorruption/NewTransient/NewConflict` constructors. `ErrHelp`/`ErrVersion` remain plain `errors.New` (control-flow signals, not domain errors). All `fmt.Errorf` wrapping sites in `pnpm.go`, `packagejson.go`, `engine.go`, `config.go`, `render.go`, `cmd/upd/main.go` use `errorfamily.Wrap*` or `sentinel.WithContext()`. Exit codes derived from `Family.ExitCode()` via `errorfamily.ExitCode(err)`. The `retryableError` wrapper in `pnpm.go` stays (carries `retryAfter` which errorfamily doesn't model).
